...
dfData = pd.read_csv('trained.csv')
dX = dfData.iloc[:, :-1].values
X=pd.DataFrame(dX)
...
def getHof():
	...
	#this line print gen and nevals
	...
	#cxpb	= the probablility of mating 2 individuals
	#mut 	= the probablility of mutating an individual
	#ngen    = the number of generation
 	#stats   = the statistics object that is updated inplace 
	#HallOfFame = the halloffame object whcih contains the best individuals
	#verbose = whetehr to log or to not to log statistics
...
if __name__ == '__main__':
	...
	for index in range(len(maxValAccSubsetIndicies)):
		print('\n\nPercentile: \t\t\t' + str(percentileList[maxValAccSubsetIndicies[index]]))
	...
	plt.title('Validation Set Classification Points')
	plt.xlabel('Population Ordered By Increasing Test Set Accuracy')
	plt.ylabel('Validation Set Accuracy')
	f.show()
	...
	plt.title('Test Set Classification Points')
	plt.xlabel('Population Ordered By Increasing Test Set Accuracy')
